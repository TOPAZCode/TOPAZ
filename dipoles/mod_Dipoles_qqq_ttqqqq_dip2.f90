...
! for qq initial state
      module mod_qqq_ttqqqq_dip
      use ModAmplitudes
      use ModMisc
      use ModProcess
      use ModIntDipoles
...
!   + bar q1(p5) + q1 (p6)
!  and here, therefore, 3 and 5 will refer to the inital state
!  and 4 and 6 are in the final state


      subroutine qqq_ttqqqq_dip(p,yRnDK1,Wgt,sum_dip)
      ...
      real(dp) :: res
      integer ::  dip(ndip,3)
      data dip(1,1)/4/ , dip(1,2)/3/, dip(1,3)/5/
      data dip(2,1)/6/ , dip(2,2)/5/, dip(2,3)/3/
      real(dp) :: mass(6)
...
!-- now Lorentz transform
     ...
     q(:,j) = k1
        ...
        if (i.eq.4) then     ! this is needed for jet staff, to put
