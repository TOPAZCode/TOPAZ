...
! for qbq initial state
      module mod_qqb_ttqqqq_dip
      use ModAmplitudes
      use ModMisc
      use ModProcess
      use ModIntDipoles
...
!----- notation for subroutines
      public :: qqb_ttqqqq_dip_12, qqb_ttqqqq_dip_11
      real(dp), private :: yRnDK(1:8), Wgt_ext

      contains

!-----  the two routines ..  dip_11 and dip_12 are for same flavor/diff flavor
!-----  qqb (initial state) scattering

!---- this is the description of the routine 11 where I assume that
!---- same flavor q & qbar are in the initial state
!  I will label things as 0-> bar t(p1)+t(p2) + bar q(p3)+q (p4)
!   + bar q1(p5) + q1 (p6)
!  and here, therefore, 3 and 4 will refer to the inital state
!  and 5 and 6 are in the final state


      subroutine qqb_ttqqqq_dip_11(p,yRnDK1,Wgt,sum_dip)
      real(dp), intent(out) ::  sum_dip
      real(dp), intent(in) :: p(4,6)
      real(dp), intent(in) :: yRnDK1(1:8), Wgt
      integer, parameter :: ndip = 1
      integer, parameter :: in1 = 3
      integer, parameter :: in2 = 4
      real(dp) :: res
      integer ::  dip(ndip,3)
      data dip(1,1)/5/ , dip(1,2)/6/, dip(1,3)/3/
      real(dp) :: mass(6)
...
!------ here only final-initial dipole contributes, so we change it

      if (j.ne.in1.and.j.ne.in2.and.k.ne.in1.and.k.ne.in2) &
           print *, 'ff dipole not implemented qqb_ttqqq_dip.f90, 11'
!    call dipff(n,i,j,k,mass(i),mass(j),mass(k),fl(i),fl(j),p,res)
    ...
    call dipfi(n,i,j,k,mass(i),mass(j),mass(k),fl(i),fl(j),p,res)
      ...
      if ( (k.ne.in1.and.k.ne.in2).and.(j.eq.in1.or.j.eq.in2)) &
           print *, 'if dipole not implemented qqb_ttqqq_dip.f90, 11'
!     call dipif(n,i,j,k,mass(i),mass(j),mass(k),fl(i),fl(j),p,res)
    ...
    call dipii(n,i,j,k,mass(i),mass(j),mass(k),fl(i),fl(j),p,res)
      ...
      end subroutine qqb_ttqqqq_dip_11




!--- this is description of the routine 12
!  so, we assume that there is a list of dipoles that we need to go through
!  I will label things as 0-> bar t(p1)+t(p2) + bar q(p3)+q (p4)
!   + bar q1(p5) + q1 (p6)
!  and here, therefore, 3 and 6 will refer to the inital state
!  and 4 and 5 are in the final state


      subroutine qqb_ttqqqq_dip_12(p,yRnDK1,Wgt,sum_dip)
      real(dp), intent(out) ::  sum_dip
      real(dp), intent(in) :: p(4,6)
      real(dp), intent(in) :: yRnDK1(1:8), Wgt
      ...
      real(dp) :: res
      integer ::  dip(ndip,3)
      data dip(1,1)/4/ , dip(1,2)/3/, dip(1,3)/6/
      data dip(2,1)/5/ , dip(2,2)/6/, dip(2,3)/3/
      real(dp) :: mass(6)
!-----flavor list qm -- massive quark, qu -- massless quark
!----- gl -- gluon
      character :: fl(6)*2
...
!     mass assignment
      ...
      if (j.ne.in1.and.j.ne.in2.and.k.ne.in1.and.k.ne.in2) &
           print *, 'ff dipole not implemented qqb_ttqqq_dip.f90, 12'
!    call dipff(n,i,j,k,mass(i),mass(j),mass(k),fl(i),fl(j),p,res)
    ...
    call dipfi(n,i,j,k,mass(i),mass(j),mass(k),fl(i),fl(j),p,res)
      ...
      if ( (k.ne.in1.and.k.ne.in2).and.(j.eq.in1.or.j.eq.in2)) &
           print *, 'if dipole not implemented qqb_ttqqq_dip.f90, 12'
!     call dipif(n,i,j,k,mass(i),mass(j),mass(k),fl(i),fl(j),p,res)
    ...
    call dipii(n,i,j,k,mass(i),mass(j),mass(k),fl(i),fl(j),p,res)
      ...
      end subroutine qqb_ttqqqq_dip_12
...
!       momentum mapping
       ...
       if (i.eq.5) then
          ...
          in1 = 3   ! those in1 and in2 refer to the position of initial
          in2 = 5   ! state vectors in the q-array
