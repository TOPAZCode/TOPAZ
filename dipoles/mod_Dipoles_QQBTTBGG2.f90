...
!----- this is for qq initial state; so 5 and 6 are in the final
!----- state ;
!----- the list of dipoles is the same as for ttgggg  for the qq channel

      subroutine EvalDipoles_QQBTTBGG(p,yRnDk1,Wgt,sum_dip)
      implicit none
...
!       endif
       ...
       complex(dp) :: cres
       integer :: oh(6,5)
       real(dp) :: mij,mui,muj,muk,q2sqrt,yp
       real(dp) :: C(4,4)
       ...
       res = zero
       cres = (0d0,0d0)
       Bm =   (0d0,0d0)  ! we need this to make sure that
                         ! parts of arrays that are not calculated for
                         ! top decays do not cause trouble
...
!-----------------     initial   initial    final  top      top
  call Kinematics_TTBARJET(0,(/-q(1:4,3),-q(1:4,4),q(1:4,5),q(1:4,1),q(1:4,2)/),MomDK(1:4,1:6),Not_Passed_Cuts,NBin(1:NumHistograms))
...
!       momentum mapping
       ...
       if (TopDecays.ge.1) then
         ...
         call EvalPhasespace_TopDecay(q(1:4,2),yRnDk(5:8),.false.,MomDK(1:4,4:6),PSWgt2)
!           print *, MomDK(1:4,1)+MomDK(1:4,2)+MomDK(1:4,3)+MomDK(1:4,4)+MomDK(1:4,5)+MomDK(1:4,6)+q(1:4,5)  +q(1:4,3)+q(1:4,4)
       elseif(TopDecays.eq.0) then
         PSWgt1 = one
         PSWgt2 = one
!           print *, q(1:4,1)+q(1:4,2)+q(1:4,5)  +q(1:4,3)+q(1:4,4)
       else
          print *, 'topdecays are not defined'
          pause
       endif
...
!     print *, "ptilde(tb)",get_pt(MomDK(1:4,1)),get_pt(MomDK(1:4,4))
