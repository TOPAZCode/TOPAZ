...
!----------------------------
      ...
      subroutine qbg_ttqqgg_dip_int(p,res)
...
!---- quark and gluon PDFs contribute here
...
! init tree processes for 0-> tb t bq q g g
   ...
   do n=1,20
    ...
    mtrs = mtrs + C(i,j)*real(AM(i,j),dp)
      ...
      if(n.eq.1) then
      dipsoft =ff_qq(zero,mt,p,3,1,z,1)
      dipfini =ff_qq(zero,mt,p,3,1,z,2)
      dipplus =ff_qq(zero,mt,p,3,1,z,3)
      diptype ='ff'
      emi = 3 ! final state dipole
      endif
      if(n.eq.2) then
      dipsoft =ff_qq(zero,mt,p,3,2,z,1)
      dipfini =ff_qq(zero,mt,p,3,2,z,2)
      dipplus =ff_qq(zero,mt,p,3,2,z,3)
      diptype ='ff'
      emi = 3 ! final state dipole
      endif
      if(n.eq.3) then
     ...
     emi = 1 ! mom #3 is emitting
      ...
      if(n.eq.10) then
      dipsoft =ff_qq(mt,zero,p,1,3,z,1)
      dipfini =ff_qq(mt,zero,p,1,3,z,2)
      dipplus =ff_qq(mt,zero,p,1,3,z,3)
      diptype ='ff'
      emi = 3 ! final state dipole
      endif
      if(n.eq.11) then
      ...
      if(n.eq.14) then
      dipsoft =ff_qq(mt,zero,p,2,3,z,1)
      dipfini =ff_qq(mt,zero,p,2,3,z,2)
      dipplus =ff_qq(mt,zero,p,2,3,z,3)
      diptype ='ff'
      emi = 3 ! final state dipole
      endif
      if(n.eq.15) then
...
!---- now add additional two dipoles for collinear singularities associated with
!---- collinear quark
...
!---- we need to rearrange momenta here
      ...
      dipsoft =ii_qg(zero,zero,p,4,5,z,1)
      dipfini =ii_qg(zero,zero,p,4,5,z,2)
      dipplus =ii_qg(zero,zero,p,4,5,z,3)
      diptype ='ii'
      emi = 1 ! mom #3 is emitting
...
! init tree processes for 0-> tb t qq g
      ...
      dipsoft =ii_gq(zero,zero,p,5,4,z,1)
      dipfini =ii_gq(zero,zero,p,5,4,z,2)
      dipplus =ii_gq(zero,zero,p,5,4,z,3)
      diptype ='ii'
      emi = 2 ! mom #4 is emitting
  ...
  end subroutine qbg_ttqqgg_dip_int

end  module qbg_ttqqgg_dipint
