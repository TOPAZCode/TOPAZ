...
implicit none
integer :: n,limit,c,AllocStatus
integer, allocatable :: TheList(:)
integer, allocatable :: TmpList(:)
integer, allocatable :: Partition(:,:)
integer :: i

   allocate(TmpList(1:size(TheList)),stat=AllocStatus)
   if( AllocStatus .ne. 0 ) print *,"Error in memory allocation for TmpList(1:n)"

   if( n.gt.0 ) then
      do i=min(n,limit),1,-1
         TmpList(1:size(TheList)) = AppendToList(TheList(1:size(TheList)),i)
         call CalcIntPartition(n-i,i,TmpList,c,Partition)
      enddo
   else
      Partition(c,1:size(TheList)) = TheList(1:size(TheList))
      c=c+1
   endif
return
END SUBROUTINE
