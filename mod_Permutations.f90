! Module Permutations
! GetPermutation(Set,PermSet,NoDblCount): returns array 'PermSet(:,:)' which contains all permutations of the set 'Set(:)'
!                                         'PermSet(NPerm,Col)' has to be allocated before the call, 'NPerm'=number of permutations, 'Col'=elements of the permutation
!                                         'NoDblCount' is an optional logical argument which allows to remove identical permutations (if'NoDblCount'=.true.)
!
...
public  :: GetPermutation,Factorial
private :: CalcPermutation,Remove
...
SUBROUTINE GetPermutation(Set,PermSet,NoDblCount,NumPerm)
implicit none
integer, allocatable :: Set(:),PermSet(:,:),PermVec(:)
logical, OPTIONAL :: NoDblCount
integer, OPTIONAL :: NumPerm
