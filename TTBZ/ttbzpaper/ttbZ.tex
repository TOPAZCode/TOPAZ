...
No mention has yet been made of theoretical (scale+pdf) uncertainty. Following ref.~\cite{Lykken et al}, we do not change the predicted number of events in each bin $\nu_i$. Instead, the observed number of events is multiplied by an uncertainty factor which is uniformly distributed between $1/(1+\Delta N)$ and $1+\Delta N$. The effect of this is to broaden the distributions of $\Lambda$, resulting in a larger overlap between the two distributions, and consequently a larger $\alpha$ value.
