...
New physics contributions to the top-$Z$ couplings may be parametrized by higher dimensional operators in the language of effective field theory. 
Assuming gauge-invariant dimension-six operators, the fully general Lagrangian may be written as \cite{AlcarazMaestre:2012vp,0811.3842}
\begin{equation} \label{L_NP}
\mathcal{L}_{\ttbZ} = \mathrm{i} e \bar{u}(p_t)\biggl[ \gamma^{\mu} \bigl(C_{1,V} + \gamma_5 C_{1,A} \bigr) + \frac{\mathrm{i} \sigma_{\mu \nu} q_{\nu}}{M} 
\bigl(C_{2,V} + \mathrm{i} \gamma_5 C_{2,A} \bigr) \biggr] v(p_{\bar{t}}) Z_{\mu},
\end{equation}
where $\sigma_{\mu \nu}=\frac{i}{2} [ \gamma_{\mu},\gamma_{\nu} ]$, $q_{\nu} = (p_{t}-p_{\bar{t}})_{\nu}$ and the 
% second term is suppressed by a  MARKUS comment: is is not really supressed because q^2/MZ~1, the M is just making it dimensionless
mass scale $M$ is typically chosen to be $M_Z$. 
The couplings $C_{2,A}$ and $C_{2,V}$ are related to the weak electric and magnetic dipole moments of the top quark, and are zero at tree-level in the SM. 
...
To do so, we will need to compute the total cross-section and differential distributions for a large number of values of both $C_{1,V}$ and $C_{1,A}$. 
...
for the non-SM coupling choice $(\delta_V,\delta_A)=(1.00,0.1)$, and from the fit described in Eq.~3.8. }
\end{figure}
...
\caption{ \label{fig:vii} Relative deviations of the NLO QCD cross section as a function of relative shifts in vector and axial couplings wrt. to the SM.
...
Note that the remaining scale unceratinty at NLO QCD was found to be $\sim 15\%$ which roughly corresponds to the lightest color band/area inside the dotted band in Fig.~\ref{fig:vii}.
...
from the fact that the LO cross section is proportional to $\ConeV^2+\ConeA^2$ and $\Delta\ConeV=-1$ corresponds to the point $\ConeV=0$.
