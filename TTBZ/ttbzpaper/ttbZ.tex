...
opens up new avenues for % more sophisticated 
...
The squared amplitudes for Born and real emission corrections are checked against {\tt MadGraph v.2.49} ~\cite{Stelzer:1994ta}. 
...
\be  
 ...
 \\
  ...
  \nonumber
\ee
and $t_\mathrm{R,L}$ are the top quark spinors and $\phi$ is the SM Higgs doublet. 
...
%Clearly, $t_\mathrm{R,L}$ are the top quark spinors and $\phi$ is the SM Higgs doublet. 
...
%We also have to mention another experimental constraint.
...
defines the acceptance function $A$, for which we find \be
A^\mathrm{LO} =
...
= 30.0 \%.  \ee The increase of approximately $3\%$ when going from
...
The differential $K$-factor in the lower pane of this plot shows shape changes in the range of 10~\% due to higher order corrections, which makes the $\pT^Z$ spectrum harder and decreases the opening angle between the leptons.
...
% Instead, the alternative hypothesis can be replaced by observed data from LHC experiments to test for consistency with the SM prediction. 
% Alternatively, the null hypothesis could then also be replaced by  other points in $(\DConeV,\DConeA)$ parameter space, and tested against the data. 
...
\end{split}
\ee
This test statistic can now be evaluated with $\vec{n}_{\mrm{obs}}$ being the Poisson distributed events from the  
...
We will choose the constant values $\Delta_\mathrm{theor.\,unc.} = 30\%$ at LO
and $\Delta_\mathrm{theor.\,unc.} = 15\%$ at NLO QCD for our $\sqrt{s}=13$~TeV analysis below.
...
LEP measurements of the $Zb_\mathrm{L}\bar{b}_\mathrm{L}$ interaction when $\mathrm{SU(2)}_\mathrm{L}$ symmetry is invoked, and consequently we do no show the results for 
...
Let us now use current LHC data to obtain direct constraints on vector and axial couplings. The production of $\ttbZ$ has been observed at the $\sqrt{s}=7$ TeV run at the LHC, 
...
Our null hypothesis is derived from the experimental cross section $\sigma_{\ttbZ}^{\mathrm{CMS}}=0.28$~pb, from which we predict 24 events in the trilepton channel 
from $5~\invfb$ of data if no acceptance cuts are applied. 
We are adopting this number as our reference point against which we compare the alternative hypothesis, through its predicted number of events in this dataset.
% Note that CMS only observes 9 events with this dataset, a value that is consistent with our acceptance of 30\% quoted in section \ref{sec:NLOres}. 
% Because we assume more events than are observed, our bounds artificially improved through better statistics. 
% Our alternative hypothesis is a point in $(\ConeV,\ConeA)$ parameter space, for which we calculate the cross-section and hence the expected number of 
% events in the $5~\invfb$ dataset. 
% This number, togeter with the 24 events expected given the cross-section measured by CMS, comprise our only inputs into the log-likelihood function of 
% equation \eqref{lilifunct}. 
As described before, it is obtained from our theoretical prediction for various points in $(\ConeV,\ConeA)$ parameter space.
% and hence subject to scale uncertainties, we include a uniform uncertainty function as 
% described in equations \eqref{errorfunctG} and \eqref{uniuncert}, with $\Delta_\mathrm{theor.\,unc.}=\pm 30\%$ {\bf 40\%???} at LO 
% and $\Delta_\mathrm{theor.\,unc.}=\pm 15\%$ at NLO QCD. 
% On the other hand, the null hypothesis is an experimental measurement, and as such is free of scale uncertainties but has both systematic and 
% statistical uncertainties. 
% The latter are taken into account through the Poisson distribution in Eq.~(\ref{lili}). 
To account for uncertainties we include a uniform distribution spanned by the theoretical error band. 
Since we find slightly larger LO uncertainties for the pdfs at 8~TeV, we choose 
$\Delta_\mathrm{theor.\,unc.} = 40\%$ at LO and keep $\Delta_\mathrm{theor.\,unc.} = 15\%$ at NLO QCD.
% described in Eq.~(\eqref{uniuncert}) for the theoretical prediction whereas
% with $\Delta_\mathrm{theor.\,unc.}=\pm 30\%$ {\bf 40\%???} at LO 
% and $\Delta_\mathrm{theor.\,unc.}=\pm 15\%$ at NLO QCD. 
% the 
...
% This is all we need to evaluate the log-likelihood ratios for null and alternative hypothesis, and use this to build up a distribution of $\Lambda$, from which we derive  the probability of incorrectly rejecting the alternative hypothesis. This is again translated into a $\sigma$-bound, following equation \eqref{alphatosig}.
...
\begin{figure}[t]
\centering % \begin{center}/\end{center} takes some additional vertical space
\includegraphics[width=0.44\textwidth]{./CMS_LOLO_SU40_CMSerr20_sig.pdf}
\hspace{2mm}
\includegraphics[width=0.52\textwidth]{./CMS_NLO_SU15_CMSerr20_sig.pdf}
...
%{\bf MARKUS: BEGIN New uncert. treatment. Insert somewhere here}\\
%Since we include experimental data we need to slightly modify the uncertainty treament described in Eq.~(\ref{uniuncert}).
%Instead of assuming a uniform distribution for the experimental systematics we introduce a Gaussian distributed probability.
%The function $\mathcal{G}(..|..)$ in Eq.~(\ref{errorfunctG}) becomes
%\be
%  \label{Gaussuncert}
%  \mathcal{G} \left( \nu | \tilde{\nu},\tilde{\sigma} \right) = \frac{1}{\sqrt{2\pi\tilde{\sigma}^2}} \; e^{-(\nu-\tilde{\nu})^2 / (2\tilde{\sigma}^2)}, 
%\ee
...
%In this way the mean value $\nu$ in the likelihood function is normal distributed during the generation of pseudo experiments with 
%a standard deviation of $\sigma_\mathrm{exp.\,syst.}$.
%Note that statistical uncertainties are already included through the Poisson distribution in Eq.~(\ref{lili}).
%In the following we adopt the experimental systematic error of $\sigma_\mathrm{exp.\,syst.}=\pm 20\%$ quoted in Ref.~\cite{Chatrchyan:2013qca}.
%The theoretical uncertainties are included through a uniform uncertainty distribution as described in the previous Section with 
%$\Delta_\mathrm{theor.\,unc.}=\pm 30\%$ {\bf 30\%???} at LO and $\Delta_\mathrm{theor.\,unc.}=\pm 15\%$ at NLO QCD.
%
%{\bf MARKUS: END}\\
%\\
%
...
We present the results of our analysis for the future run of the LHC at $\sqrt{s}=13$~TeV in Figs.~\ref{fig:ix} and~\ref{fig:x}.
...
\ee
% 
This result should be considered with care given the low number of events observed in experiments. 
...
Note that these results are not only based on the total cross section but also on the shapes
...
This allows to contrast high precision indirect limits from electroweak observables with a direct determination from the LHC.
...
\acknowledgments
We acknowledge helpful conversations with P.~Agrawal, J.~Campbell, K.~Ellis, Y.~Gao and N.~Tran. R.R. is supported by the US DOE under contract DE-AC02-06CH11357. This research used resources of the National Energy Research Scientific Computing Center, which is supported by the Office of Science of the U.S. Department of Energy under Contract No. DE-AC02-05CH11231.
