...
We choose the constant values $\Delta_\mathrm{theor.\,unc.} = 30\%$ at LO
and $\Delta_\mathrm{theor.\,unc.} = 15\%$ at NLO QCD.
...
Let us now use current LHC data to obtain first direct constraints on vector and axial couplings. The production of $\ttbZ$ has been observed at the $\sqrt{s}=7$ TeV run at the LHC, 
...
Clearly, error bars from these very first measurements are large. 
Nevertheless they are consistent with the NLO QCD predictions of $0.137\,$pb$\,\pm 11\%$ in Ref.~\cite{Garzelli:2011is} (CTEQ pdfs) 
or with $0.148\,$pb$\,\pm 11\%$ from our calculation (MSTW pdfs). 
In spite of the low number of events and correspondingly high statistical error, it is instructive to use this overall cross section to place bounds on the $\ttbZ$ couplings. 
This constitutes the first direct constraints on these couplings.

{\bf MARKUS: BEGIN change text below}\\
We perform a log-likelihood analysis, as described in section \ref{sect:analysis}. Our null hypothesis is derived from the experimental cross-section $\sigma_{\mathrm{CMS}}=0.28$pb, from which we predict 24 events in the trilepton channel from $5~\invfb$ of data. Note that CMS only observes 9 events with this dataset, a value that is consistent with our acceptance of 30\% quoted in section \ref{sec:NLOres}. Because we assume more events than are observed, our bounds artificially improved through better statistics. Our alternative hypothesis is a point in $(\ConeV,\ConeA)$ parameter space, for which we calculate the cross-section and hence the expected number of events in the $5~\invfb$ dataset. This number, togeter with the 24 events expected given the cross-section measured by CMS, comprise our only inputs into the log-likelihood function of equation \eqref{lilifunct}. 

Since our alternative hypothesis is a theoretical prediction, and hence subject to scale uncertainties, we include a uniform uncertainty function as described in equations \eqref{errorfunctG} and \eqref{uniuncert}, with $\Delta_\mathrm{theor.\,unc.}=\pm 30\%$ {\bf 40\%???} at LO and $\Delta_\mathrm{theor.\,unc.}=\pm 15\%$ at NLO QCD. On the other hand, the null hypothesis is an experimental measurement, and as such is free of scale uncertainties but has both systematic and statistical uncertainties. The latter are taken into account through the Poisson distribution in Eq.~(\ref{lili}). To include the systematic uncertainties, we introduce a Gaussian distributed probability. The function $\mathcal{G}(..|..)$ in Eq.~(\ref{errorfunctG}) becomes
\be
  \label{Gaussuncert}
...
\hspace{2mm}
\includegraphics[width=0.44\textwidth]{./CMS_NLO_SU15_CMSerr20_sig.pdf}
...
The constraints from the data using a LO calculation are $ -13 \lesssim \DConeV \lesssim 12$ and $-4 \lesssim \DConeA \lesssim 2$ at 3-$\sigma$, while the NLO constraints are $-10 \lesssim \DConeV \lesssim 8$ and $-3.5 \lesssim \DConeA \lesssim  1.5$.
...
for the future run of the LHC at $\sqrt{s}=13$~TeV. 
...
Using a larger event sample, shape sensitivity increases and also allows to exclude regions where normalization differences 
are small but shapes differ significantly.
This is the case in the lower right plot of Fig.~\ref{fig:ix}, where the previously downwards bend are is safely excluded.
...
It would be an interesting continuation of this work to superimpose Fig.~\ref{fig:x} with constraints obtained from single $t Z+ \bar{t}Z$ production at the LHC.
