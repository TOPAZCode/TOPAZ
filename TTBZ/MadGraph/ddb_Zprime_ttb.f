      SUBROUTINE SDDB_ZPRIME_TTB(P1,ANS)
C  
...
C  
C FOR PROCESS : d d~ -> t t~  
C  
C Crossing   1 is d d~ -> t t~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      DO IHEL=1,NCOMB
          IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
             T=DDB_ZPRIME_TTB(P ,NHEL(1,IHEL),JC(1))            
...
C             WRITE(*,*) IHEL,T
      ...
      REAL*8 FUNCTION DDB_ZPRIME_TTB(P,NHEL,IC)
C  
C FUNCTION GENERATED BY MADGRAPH
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL JIOXXX(W(1,1   ),W(1,2   ),GZD ,ZMASS   ,ZWIDTH  ,W(1,5   ))    
      CALL IOVXXX(W(1,4   ),W(1,3   ),W(1,5   ),GZU ,AMP(1   ))            
      JAMP(   1) = -AMP(   1)
      DDB_ZPRIME_TTB = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          ...
          ENDDO
          DDB_ZPRIME_TTB =DDB_ZPRIME_TTB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
