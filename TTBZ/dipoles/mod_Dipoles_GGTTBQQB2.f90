...
!  and here, therefore, 5 and 6 will refer to the inital state
!  and 3 and 4 are in the final state


      subroutine EvalDipoles_GGTTBQQB(p,yRnDk1,Wgt,sum_dip)
      real(dp), intent(out) ::  sum_dip
      real(dp), intent(in) :: p(4,6)
      real(dp), intent(in) :: yRnDK1(1:8), Wgt
      integer, parameter :: ndip = 5
      integer, parameter :: in1 = 5
      integer, parameter :: in2 = 6
      real(dp) :: res
      integer ::  dip(ndip,3)
      data dip(1,1)/3/ , dip(1,2)/4/, dip(1,3)/5/
      data dip(2,1)/3/ , dip(2,2)/5/, dip(2,3)/6/
      data dip(3,1)/3/ , dip(3,2)/6/, dip(3,3)/5/
      data dip(4,1)/4/ , dip(4,2)/5/, dip(4,3)/6/
      data dip(5,1)/4/ , dip(5,2)/6/, dip(5,3)/5/
      real(dp) :: mass(6)
...
!      dipole subroutines
       ...
       real(dp) ::  pijt(4),yp,q2sqrt,muk,mui,muj
...
!-----------------     initial   initial       final  top      top
   call Kinematics_TTBARJET(0,(/-q(1:3,3),-q(1:4,4),q(1:4,5),q(1:4,1),q(1:4,2)/) &
...
!       momentum mapping
        ...
        if( alpha_fi.lt.one-xija ) then   ! agrees with MCFM implementation
            res = 0d0
            return
        endif
...
!------- fill in histograms
       ...
       real(dp) :: xx1, xx2, zi, zj, yijk, vijk, zim
       real(dp) :: pij2,pijk2,tvijk,q2,tpij2, qij2, zjm
       real(dp) ::  pijt(4), pija(4), pija2, xija,u
...
!       momentum mapping
        ...
        u = scr(pa,pi)/xxx
        if( alpha_if.lt.u ) then   ! agrees with MCFM implementation
            res = 0d0
            return
        endif
...
!       momentum mapping
        ...
        if( alpha_ii.lt.v ) then   ! agrees with MCFM implementation
            res = 0d0
            return
        endif
...
!     now build the helicity matrix
      ...
      diag= two*(xiab/(one-xiab)+xiab*(one-xiab))   ! HERE WAS A BUG: two*xiab/(one-xiab)+xiab*(one-xiab)
