...
!
      ...
      SUBROUTINE EvalDipoles_DKJ_GGTTBG(Mom,Mass2,yRnd,Wgt,DKTop_,nPhoRad_,ResultSum)
...
!       SqAmp = Tree_GG_TTb_453_5(dcmplx(TheDipole%MomTd),TheDipole%Mass2Td,Split_A,Split_B,Split_V)
      ...
      integer col,colP,iHel,icorr,ngl,PhoHel
      ...
      if(DKTop.eq.ATop_) then
            if( nPhoRad.eq.1 ) then
              call EvalPhasespace_TopDecay(MomExt(1:4,3),yRndDK(1:7),.true.,MomExt(1:4,5:8),PSWgt2)!  here we use momext rather than momtd (3<-->4)!
            else
...
!      sum over helicities
      ...
      if(DKTop.eq.ATop_) then
            if( nPhoRad.eq.1 ) then
              call TopDecay(TopQuark(2),DKJ_LO_T,MomExt(1:4,5:8),GluonHel=PhoHel)
            else
              call TopDecay(TopQuark(2),DKJ_LO_Q,MomExt(1:4,5:8),GluonHel=PhoHel)
            endif
            call TopDecay(TopQuark(1),DK_LO,MomExt(1:4,9:11))
      else
            call TopDecay(TopQuark(2),DK_LO,MomExt(1:4,5:7))
            if( nPhoRad.eq.1 ) then
              call TopDecay(TopQuark(1),DKJ_LO_T,MomExt(1:4,8:11),GluonHel=PhoHel)
            else
              call TopDecay(TopQuark(1),DKJ_LO_Q,MomExt(1:4,8:11),GluonHel=PhoHel)
            endif
      endif
