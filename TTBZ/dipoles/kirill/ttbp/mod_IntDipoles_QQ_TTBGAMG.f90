module ModIntDipoles_QQ_TTBGAMG
use ModParameters
use ModProcess
use ModMisc
use colorcorr_ttgam
...
public:: EvalIntDipoles_QQ_TTBGAMG
...
!-----------------------------------------------------------
      ...
      subroutine EvalIntDipoles_QQ_TTBGAMG(p,MomDK,z,res)
      use ModIntDipoles2
      use ModAmplitudes
      implicit none
...
! spin & color avg., color convention adjustment
     ...
     mtrsq = mtrsq * alpha_s4Pi**2 * alpha4Pi * Q_top**2 /4d0/9d0
...
! !        epcorr=epinv+2d0*dlog(renscale/facscale)
       ...
       AP(2,1:3) = AP(2,1:3) * alpha_sOver2Pi *epcorr * mtrsq(2)
        ...
        res(1:2,1) = res(1:2,1) + 2d0*AP(1:2,1)-AP(1:2,3)
