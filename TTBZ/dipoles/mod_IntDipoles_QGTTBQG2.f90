...
!----------------------------
      ...
      subroutine EvalIntDipoles_QGTTBQG(p,MomDK,z,res)
      use ModAmplitudes
      implicit none
...
! init tree processes for 0-> tb t bq q g g
   ...
   do n=1,20
     ...
     emi = 1 ! mom #3 is emitting
      ...
      if(n.eq.5) then
      dipsoft =ff_qq(zero,m_Top,p,4,1,z,1)
      dipfini =ff_qq(zero,m_Top,p,4,1,z,2)
      dipplus =ff_qq(zero,m_Top,p,4,1,z,3)
      diptype ='ff'
      emi = 3 ! final state dipole
      endif
      if(n.eq.6) then
      dipsoft =ff_qq(zero,m_Top,p,4,2,z,1)
      dipfini =ff_qq(zero,m_Top,p,4,2,z,2)
      dipplus =ff_qq(zero,m_Top,p,4,2,z,3)
      diptype ='ff'
      emi = 3 ! final state dipole
      endif
      if(n.eq.7) then
      ...
      if(n.eq.11) then
      dipsoft =ff_qq(m_Top,zero,p,1,4,z,1)
      dipfini =ff_qq(m_Top,zero,p,1,4,z,2)
      dipplus =ff_qq(m_Top,zero,p,1,4,z,3)
      diptype ='ff'
      emi = 3 ! final state dipole
      endif
      if(n.eq.12) then
      ...
      if(n.eq.15) then
      dipsoft =ff_qq(m_Top,zero,p,2,4,z,1)
      dipfini =ff_qq(m_Top,zero,p,2,4,z,2)
      dipplus =ff_qq(m_Top,zero,p,2,4,z,3)
      diptype ='ff'
      emi = 3 ! final state dipole
      endif
      if(n.eq.16) then
...
!           print *, "MG ratio1",mtrs/MadGraph_tree
      ...
      dipsoft =ii_qg(zero,zero,p,3,5,z,1)  *2d0*CF
      dipfini =ii_qg(zero,zero,p,3,5,z,2)  *2d0*CF
      dipplus =ii_qg(zero,zero,p,3,5,z,3)  *2d0*CF
      diptype ='ii'
      emi = 1 ! mom #3 is emitting
      if(emi.eq.1) then
...
!           pause
      ...
      dipsoft =ii_gq(zero,zero,p,5,3,z,1) *2d0  ! factor 2d0 fixes normalization for ii_gq dipole
      dipfini =ii_gq(zero,zero,p,5,3,z,2) *2d0
      dipplus =ii_gq(zero,zero,p,5,3,z,3) *2d0
      diptype ='ii'
      emi = 2 ! mom #4 is emitting
      if(emi.eq.2) then
        res(1) = res(1) + (dipsoft-dipplus)*mtrs * alpha_sOver2Pi*0.5d0   ! overall normalization: alpha_s/4pi
