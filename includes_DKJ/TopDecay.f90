...
subroutine calc_topdecay(pt,kb,kneu,klep,kg,pol_g,xe,mu,lo_nlo,gau,Spinor)
implicit none
real(8):: pt(1:4),kb(1:4),klep(1:4),kneu(1:4),kg(1:4), mu,ThetaEps
complex(8) :: Spinor(1:4), CT(1:4), CTMass(1:4), weyl(1:4), CoeffsTopNLO(1:8)
...
call SPandPoltop(pt,kb,kneu,klep,kg,pol_g, gau)
if(LO_NLO .eq.  0) then
   call CalcSMEtop
...
subroutine calc_atopdecay(pt,kb,kneu,klep,kg,pol_g,xe,mu,lo_nlo,gau,Spinor)
implicit none
real(8):: pt(1:4),kb(1:4),klep(1:4),kneu(1:4),kg(1:4), mu,ThetaEps
complex(8) :: Spinor(1:4), CT(1:4), CTMass(1:4), weyl(1:4), CoeffsAtopNLO(1:8)
integer :: pol_g, lo_nlo, gau, xe, i
Spinor(1:4) = zero
Nc =3.d0
DenNc =1.d0/3.d0
...
call SPandPolAtop(pt,kb,kneu,klep,kg,pol_g, gau)
if(LO_NLO .eq.  0) then
   call CalcSMEAtop
...
subroutine SPandPOLAtop(pt,kb,kneu,klep,kg,pol_g,gau)
implicit none
integer :: gau,pol_g, i, pol_w
...
subroutine SPandPOLTop(pt,kb,kneu,klep,kg,pol_g,gau)
implicit none
integer :: pol_g, gau, i, pol_w
real(8) ::  pt(1:4),kb(1:4),klep(1:4),kneu(1:4),kg(1:4)
do i =1,4
...
p1q = dble(-2.d0*(SP4(6) - SP4(7)))
       m1q = mzero
       m2q = mzero
       Integrals(4) = qlI2(p1q,m1q,m2q,muq,xe)

p1q = dble(mt**2 - 2.d0*SP4(7))
       m1q = mt**2
       m2q = mzero
       Integrals(5) = qlI2(p1q,m1q,m2q,muq,xe)
...
p1q = dble(mzero)
       ...
       p3q = dble(mzero)
       p4q = dble(mt**2)
       p12q = dble(Mw**2 + 2.d0*SP4(6))
       p23q = dble(mt**2 - 2.d0*SP4(7))
...
CT(1:4) = zero
  ...
  if(xe .eq. 0 .or. xe .eq. -1) then
     ...
     call CTgsATop
     if(xe .eq. 0 ) then
        dZMtop = 1.d0/2.d0*(DenNc-Nc)*m_top*alpha_S/4.d0/Pi*(3.d0* dlog(mu**2/m_top**2) + 4.d0)
        dZgsTop = alpha_S/12.d0/Pi*dlog(mu**2/m_top**2)
        dZgluonTop = -alpha_S/6.d0/Pi*dlog(mu**2/m_top**2)
        dZwave_top = GS**2/(4.d0*Pi)**2* 1.d0/2.d0*(Nc-DenNc)*(-4.d0 -3.d0*dlog(mu**2/m_top**2))
     elseif(xe .eq. -1 ) then
        dZMtop = 1.d0/2.d0*(DenNc-Nc)*m_top*alpha_S/4.d0/Pi*3.d0
        dZgsTop    = alpha_S/4.d0/Pi*(N_f/3.d0-11.d0/2.d0) + alpha_S/12.d0/Pi
        dZgluonTop = -alpha_S/6.d0/Pi
...
CT(1:4) = zero
 ...
 CT(1:4) =  amp_weyl_ctwave(1:4) + amp_weyl_ctgsTop(1:4)+amp_weyl_ctgluonTop(1:4)
      ...
      *t6)*t33
      ...
      t23*t6)*t33/2.d0
      CoeffsTopCTgluon(5) = 0
      CoeffsTopCTgluon(6) = 0
      CoeffsTopCTgluon(7) = 0
      ...
      t28*t6)*t37/2.d0
      CoeffsAtopCTwave(5) = 0
      CoeffsAtopCTwave(6) = t1*GW*mt*t46/2.d0
      CoeffsAtopCTwave(7) = 0
      ...
      *t28*t6)*t37/2.d0
      CoeffsAtopCTgluon(5) = 0
      CoeffsAtopCTgluon(6) = t1*GW*mt*t46/2.d0
      CoeffsAtopCTgluon(7) = 0
