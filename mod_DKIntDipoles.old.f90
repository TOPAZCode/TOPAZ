...
FUNCTION  DKff_qq(xe,m2,sijk)!  NT eq.(22)
implicit none
real(8) :: DKff_qq,DKff_qq_xe(-2:0),m2,sijk,L,L2
integer :: xe
...
FUNCTION  DKff_qg(xe,m2,sijk)!  NT eq.(23)
implicit none
real(8) :: DKff_qg,DKff_qg_xe(-2:0),m2,sijk,L,L2
integer :: xe
...
FUNCTION  DKff_gg(xe,m2,sijk)!  NT eq.(24)
implicit none
real(8) :: DKff_gg,DKff_gg_xe(-2:0),m2,sijk,L,L2
integer :: xe
...
FUNCTION DKfi_qq(xe,m2,sijk)!  CET eq.(38)
implicit none
real(8) :: DKfi_qq,DKfi_qq_xe(-2:0),m2,sijk,L,L2,omrsq
integer :: xe

!     L  = dlog(m2/sijk)!  note that 1-r2=sijk/m2
!     L2 = L**2
    omrsq = -sijk/m2
    ...
    DKfi_qq_xe(0)  = 25d0/4d0 + 0.5d0*(1d0/omrsq**2-8d0/omrsq+7d0)*dlog(1d0-omrsq) + 0.5d0/omrsq + 2d0*DLi2(omrsq) - 5d0*pisqo6  &
                - 5d0*dlog(omrsq) + 2d0*dlog(omrsq)**2 &!    eta=0 (FHD scheme)
...
FUNCTION DKfi_gg(xe,m2,sijk)!  ask Kirill
implicit none
real(8) :: DKfi_gg,DKfi_gg_xe(-2:0),m2,sijk,L,L2
...
!             +(17d0/12d0-dlog(1d0-r2)-1d0/2d0*L)*epinv &
        DKfi_gg_xe(-2) = 0.5d0
        DKfi_gg_xe(-1) = 17d0/12d0  -dlog(1d0-r2)-1d0/2d0*L!   maybe the log terms are wrong by a factor of 1/2
...
!       factor out (mu^2/m2)^eps
        DKfi_gg_xe(-1) = DKfi_gg_xe(-1) - dlog(MuRen**2/m2)*DKfi_gg_xe(-2)
        DKfi_gg_xe(0)  = DKfi_gg_xe(0) - 0.5d0*dlog(MuRen**2/m2)**2*DKfi_gg_xe(-2) - dlog(MuRen**2/m2)*DKfi_gg_xe(-1)
