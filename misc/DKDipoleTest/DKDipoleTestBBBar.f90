program DKDipoles!     mat.el. for t->b w bbar    and dipoles    DOES NOT WORK
use ModAmplitudes!
use ModProcess
use modMisc
implicit none
...
!           if( (Mom(1:4,3).dot.Mom(1:4,4))/mt**2 .lt. 1d-2 ) stop
          ...
          ExtParticles(1)%Mom(1:4) = dcmplx(MomT(1:4,1))!top
          ExtParticles(4)%Mom(1:4) = dcmplx(MomT(1:4,2))! w
          ...
          do hel4=1,-1,-1
          do hel3=1,-1,-2
                  ExtParticles(2)%Mom(1:4) = dcmplx(MomT(1:4,3))! bot
                  ...
                  call vSpi_Weyl(dcmplx(MomT(1:4,5)),+Hel3,Spi(1:4))
                  call ubarSpi_Weyl(dcmplx(MomT(1:4,4)),-Hel3,BarSpi(1:4))
                  ...
                  call EvalTree2(TreeAmpsDip(1),Ampl1)! s-channel




!                   t-channel
!                   exchange two b-quarks: MomT(1:4,3) <--> MomT(1:4,4)
                  ExtParticles(2)%Mom(1:4) = dcmplx(MomT(1:4,4))
                  call ubarSpi(ExtParticles(2)%Mom(1:4),ExtParticles(2)%Mass,Hel2,ExtParticles(2)%Pol(1:4))
                  call ubarSpi_Weyl(dcmplx(MomT(1:4,3)),-Hel3,BarSpi(1:4))
                  ExtParticles(3)%Mom(1:4)= dcmplx(MomT(1:4,3)+MomT(1:4,5))
                  ExtParticles(3)%Pol(1:4) = vbqq_Weyl(4,BarSpi(1:4),Spi(1:4)) * (0d0,-1d0)/( ExtParticles(3)%Mom(1:4).dot.ExtParticles(3)%Mom(1:4) )
                  call EvalTree2(TreeAmpsDip(1),Ampl2)

                  SqMat = SqMat + 8d0*(Ampl1)*dconjg(Ampl1) + 8d0*(Ampl2)*dconjg(Ampl2)
                  if( Hel3.eq.+1 ) SqMatDip = SqMatDip + 8d0/3d0*(Ampl1)*dconjg(Ampl2) + 8d0/3d0*(Ampl2)*dconjg(Ampl1)
        enddo
        enddo
        enddo
        enddo
!                                             (4pi*alpha_s)
        ...
        SqMatDip = SqMatDip * 1d0/2d0/3d0  *  (1.633628179866692484d0)**2   *0.5d0
        call ST_BWPBBB((/MomT(1:4,1),MomT(1:4,3),MomT(1:4,2),MomT(1:4,4),MomT(1:4,5)/),MadGraph_tree)

          print *, "diff",(MadGraph_tree-SqMat)
          print *, "SqMatDip",SqMatDip
          print *, "ratio",(MadGraph_tree-SqMat)/SqMatDip
!         print *, "My Amp1.:", SqMat
!         print *, "MadGraph1:", MadGraph_tree
!         print *, "MG1 ratio:", MadGraph_tree/(SqMat)
        pause
